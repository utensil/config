...
# Based on the work of Erik Johnson (xtrementl): https://github.com/xtrementl/dev-bash-git-ps1
# 
# See also http://www.thegeekstuff.com/2008/09/bash-shell-ps1-10-examples-to-make-your-linux-prompt-like-angelina-jolie/
#
# Notes:
#   The marker ($) will be colored red/green depending on the result of last command's exit code
#   Show a simplified version if user was working on the terminal in 30 seconds
...
__secs_since_prompt() {
  local now="$(date +%s)"
  local last_prompt_time="${now}"
  local exported_last_prompt_time="$(cat /tmp/__exported_last_prompt_time || echo '')"
  [ -n "${exported_last_prompt_time}" ] && last_prompt_time="${exported_last_prompt_time}"
  if [ 0 -lt "$now" ] && [ 0 -lt "$last_prompt_time" ]; then
    echo "$((now - last_prompt_time))"
  fi
}
...
__dev_prompt() {
        ...
        case "$in_gitdir" in
            ...
            *)
                ...
                if [ -n "$secs" ]; then
                  ...
                  extras="${extras}${timestr}"
                fi
            ;;
        esac
        branch="${YELLOW}${branch}${RESET}"
        branch_status="@${branch}${extras}"
    fi

    local prompt_secs="$(__secs_since_prompt)" 
    if [ -n "$prompt_secs" ] && [ 30 -lt $prompt_secs ]; then
      show_info=true
    elif history|tail -n 1|grep cd|grep -v grep > /dev/null 2>&1; then
      show_info=true
    fi
    ...
    local full_dir="${CYAN}\w${RESET}"
    timing="${GREY}\D{%Y-%m-%d} \A${RESET}"
